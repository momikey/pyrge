...
if __name__ == '__main__':
    thegame = world.World(fps=30)
    e = Entity(0,0)
    e.loadAnimation('shipstrip.png').addAnimation('stop', [0]).addAnimation('run', [1])
    e.rotating = True
    e.scroll = point.Point(1,1)

    def onkey(evt):
        if evt.key == Game.Constants.K_SPACE:
            if e.currentAnimation == 'run':
                e.play('stop')
            else:
                e.play('run')
        if evt.key == Game.Constants.K_RIGHT:
            e.angle -= 10
        elif evt.key == Game.Constants.K_LEFT:
            e.angle += 10
        elif evt.key == Game.Constants.K_UP:
            e.scale(1.1,1.1,scaleAnimations=True, smooth=True)
        elif evt.key == Game.Constants.K_DOWN:
            e.scale(1/1.1, 1/1.1, True, True)

    e2 = Tweener(x=100, y=150, w=50, h=50)
    t = tween.Tween()
    t2 = tween.Tween(0,10,2,tweenfunc.Smoothstep)
    ...
    def renderFps():
        fps = thegame.clock.get_fps()
        spr = Game.Sprite.DirtySprite()
        spr.image = Game.defaultFont.render("FPS: %.2f" % fps, False, (255,255,255), (0,0,0))
        spr.rect = spr.image.get_rect()
        spr.rect.bottomleft = (0, thegame.height)
        spr.dirty = 1
        return spr

    def renderEntities():
        numEntities = len(thegame._entities)
        spr = Game.Sprite.DirtySprite()
        spr.image = Game.defaultFont.render("Entites: %d" % numEntities, \
                                            False, (255,255,255), (0,0,0))
        spr.rect = spr.image.get_rect()
        spr.rect.bottomright = (thegame.width, thegame.height)
        spr.dirty = 1
        return spr

    fpsdisplay = renderFps()
    entitiesdisplay = renderEntities()

    def onframe():
        global fpsdisplay, entitiesdisplay
        thegame.remove((fpsdisplay, entitiesdisplay))
        fpsdisplay = renderFps()
        entitiesdisplay = renderEntities()
        thegame.add((fpsdisplay, entitiesdisplay))

    e.play('stop')
    thegame.add([e,e2])
    e.foreground()

    thegame.addHandler(Game.Constants.KEYDOWN, onkey)
    thegame.add(fpsdisplay)
    thegame.addUpdater(onframe)
    thegame.loop()
